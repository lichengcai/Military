...
package com.military.military.widget.astuetz;
...
public class MySlidingTabStrip extends HorizontalScrollView {
	...
	private int tabTextColor = 0xFF929aa2;
	...
	private boolean is_use_linear;
	private Locale locale;
	private MyLinearLayout.OnClickSortChangeListener onClickSortChangeListener;
	...
	public MySlidingTabStrip(Context context, AttributeSet attrs) {
	...
	public void setTitles(String[] titles, boolean is_use_linear) {
		this.is_use_linear=is_use_linear;
        this.titles=titles;
	...
	public void notifyDataSetChanged() {
		...
		boolean isShowImage = tabCount < 4 ? true : false;
		for (int i = 0; i < tabCount; i++) {

				if (is_use_linear) {
					addLiearTab(i, titles[i],
							isShowImage);
				}else {
...
//				});
	...
	private void addLiearTab(final int position, String title,
			boolean isShowImage) {
	...
	private void addTab(final int position, final View tab) {
		...
		tab.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (tab instanceof MyLinearLayout) {
					MyLinearLayout layout=(MyLinearLayout) tab;
					if (layout.isSelect()) {
						onClickSortChangeListener.onclickChange(position,!layout.isUp());
						layout.setImageChange(!layout.isUp());
					}else {
						layout.setSelect(true);
						onTabClickListener.onTabClick(position);
					}
				}else {
                    scrollToChild(position, (int) ( tabsContainer
                            .getChildAt(position).getWidth()));
                    selectedPosition=position;
                    currentPositionOffset=position;
                    updateTabStyles();
                    invalidate();
                    onTabClickListener.onTabClick(position);
				}
...
//			v.setBackgroundResource(tabBackgroundResId);
			...
			if (v instanceof MyLinearLayout) {
				MyLinearLayout myLinearLayout = (MyLinearLayout) v;
				myLinearLayout.textView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
						tabTextSize);
				myLinearLayout.textView.setTypeface(tabTypeface,
						tabTypefaceStyle);
				myLinearLayout.textView.setTextColor(tabTextColor);
				myLinearLayout.setSelect(false);
				if (textAllCaps) {
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
						myLinearLayout.textView.setAllCaps(true);
					} else {
						myLinearLayout.textView.setText(myLinearLayout.textView
								.getText().toString().toUpperCase(locale));
					}
				}
				if (i == selectedPosition) {
					myLinearLayout.textView.setTextColor(selectedTabTextColor);
					myLinearLayout.setSelect(true);
				}
			}
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		float lineLeft = currentTab.getLeft()+20;
		float lineRight = currentTab.getRight()-20;
		...
		if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
			...
			final float nextTabLeft = nextTab.getLeft()+20;
			final float nextTabRight = nextTab.getRight()-20;
	...
	public MyLinearLayout.OnClickSortChangeListener getOnClickSortChangeListener() {
		return onClickSortChangeListener;
	}
    public interface  OnTabClickListener {
        void onTabClick(int position);
    }
	public void setOnClickSortChangeListener(
			MyLinearLayout.OnClickSortChangeListener onClickSortChangeListener) {
		this.onClickSortChangeListener = onClickSortChangeListener;
	}
    ...
    /**
     * update lable or tab state
     * @param currentPosition
     */
    public void setCurrentPosition(int currentPosition) {
        this.currentPosition = currentPosition;
        this.currentPositionOffset=currentPosition;
        updateTabStyles();
    }
